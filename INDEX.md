...
```
code/      38 data-governance, statistical-analysis & statistical-figure scripts (Python 3.11 + one Node.js ingest script)
...
| Cluster-robust SE sensitivity for the non-FE specifications (Methods §Statistics) | `results/clustered_se_sensitivity.csv` (`code/52_clustered_se_sensitivity.py`): discrete-time model re-expressed as `CoxPHFitter(strata=stop, T=1)` — point estimates identical to the released CTV fits to 5 decimals (`HR_ctv` vs `HR_phf` columns) — with `cluster_col="id"` sandwich standard errors |
| Unadjusted (crude) estimates, four production outcomes (Methods §Statistics; STROBE 16a) | `results/unadjusted_sensitivity.csv` (`code/53_unadjusted_sensitivity.py`): MASLD-only discrete-time Cox on the same lagged risk-set definitions, no covariates |
| Baseline characteristics of the cohort (Supplementary Table S8; STROBE 14) | `results/tableS8_baseline.csv` (`code/54_baseline_table.py`): per-participant first-available-measurement aggregate statistics (median (IQR) / n (%)); individual-level source tables are institution-side and not redistributed |
...
| Statistical figures (from released result tables) | `19_figure1_flow.py`, `28_peryear_fig4.py`, `34_make_figures_v2.py` | Figures 1–4 and Supplementary Figure 1 assembled from the released statistical artefacts |
