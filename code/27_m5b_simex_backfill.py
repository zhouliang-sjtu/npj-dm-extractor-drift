...
def main():
    ...
    ax.plot(hr_t_grid, [hr_obs(h, s_c) for h in hr_t_grid], color="#d62728", lw=2.6,
            label=f"Measured κ={k:.2f} (dict v1 vs gold, n=600)")
    ...
    fig.tight_layout()
    for ext in ("png",):
        fig.savefig(os.path.join(RES, f"fig5b_simex_measured.{ext}"), dpi=300)
    plt.close(fig)
    print(f"\n-> {RES}/m5b_simex_kappa.csv, m5b_simex_backfill.csv, fig5b_simex_measured.png")
    return 0
