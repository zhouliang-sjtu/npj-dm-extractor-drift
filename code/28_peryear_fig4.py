...
def llm_ecg_any_abnormal(raw):
    ...
    注：不能用"任一异常字段"推导——2023 新模板的早期复极类报告 gold normal=0
...
def main():
    ...
    ax.set_title("Transportability: extractor–gold agreement by calendar year",
                 fontsize=10.5, fontweight="bold", loc="left", pad=10)
    ax.annotate("acceptance gate κ=0.80", xy=(0.015, 0.80 + 0.012), xycoords=("axes fraction", "data"),
                fontsize=7.6, color="#8a6d1a", ha="left")
    fig.tight_layout()
    for ext in ("png", "pdf"):   # 只写规范名，避免旧名图残留在 figures/ 根目录
        fig.savefig(os.path.join(FIGD, f"SupplementaryFigureS3_transportability.{ext}"), dpi=300,
                    bbox_inches="tight")
    plt.close(fig)
    print("-> results/m3d_peryear.csv; figures/SupplementaryFigureS3_transportability.png/pdf")
    return 0
