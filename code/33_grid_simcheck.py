...
设计：n=40,000；π=实测人均波 MASLD 患病率（H_analysis_long，与 code/06 同源）；HR_true∈{1.0,1.2,1.4}；κ∈{0.60,0.80,0.95}（解析 Se=Sp）；
...
# π 单一口径：与 code/06 网格同源，实测自 H_analysis_long（不硬编码，避免随数据修正漂移）
...
def hr_obs_analytic(hr_true, s, pi=PI):
    p1 = pi * s + (1 - pi) * (1 - s)
    p0 = 1 - p1
